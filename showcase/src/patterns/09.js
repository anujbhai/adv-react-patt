...
/*
 * Custom previous prop/state hook
 */
...
const useClapState = (initialState = INITIAL_STATE) => {
  ...
  const updateClapState = useCallback(() => {
    setClapState(({count, countTotal}) => ({
      ...
      countTotal: count < MAX_USER_CLAP
        ? countTotal + 1
        : countTotal,
    }));
  }, [count, countTotal]);
...
const ClapContainer = props => {
  const {children, setRef, handleClick, ...propsItems} = props;
...
const ClapCount = props => {
  const {count, setRef, ...propsItems} = props;
...
const CountTotal = props => {
  const {total, setRef, ...propsItems} = props;
  ...
  return (
    <div>
      ...
      <section>
        ...
        <pre className={userStyles.resetMsg}>{uploadReset ? `uploading reset ${resetDep} ...` : ""}</pre>
      </section>
    </div>
  );
  // return <MediumClap />;
