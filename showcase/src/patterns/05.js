 ...
 */
const useClapAnimation = props => {
  const { clapEl, countEl, totalEl } = props;
...
const ClapIcon = props => {
  const { style: userStyles = {}, className } = props;
...
const ClapCount = props => {
  ...
  return (
    <span ref={setRef} data-refkey="clapCountRef" id="clapCount" className={classNames} style={userStyles}>+ {count}</span>
  );
};
...
const CountTotal = props => {
  ...
  return (
    <span ref={setRef} data-refkey="clapTotalRef" id="clapCountTotal" className={classNames} style={userStyles}>{countTotal}</span>
  );
};
...
const Usage = () => {
  ...
  return (<>
    <div style={{ width: "100%" }}>
      ...
      {!!state.count && (<div className={styles.info}>
        {`You have clapped ${state.count} times`}
      </div>)}
    </div>
  </>);
};
