...
const ClapCount = props => {
  ...
  return (
    <span ref={setRef} data-refkey="clapCountRef" id="clapCount" className={styles.count} style={userStyles}>+ {count}</span>
  );
};
...
const CountTotal = props => {
  ...
  return (
    <span ref={setRef} data-refkey="clapTotalRef" id="clapCountTotal" className={styles.total} style={userStyles}>{countTotal}</span>
  );
};
  ...
  return (<>
    <div style={{ width: "100%" }}>
      <MediumClap onClap={handleClap} style={{border: '1px solid red'}}>
        <MediumClap.Icon />
        <MediumClap.Count />
        <MediumClap.Total style={{ backgroundColor: '#8cacea', padding: '7px 12px' }} />
      </MediumClap>
